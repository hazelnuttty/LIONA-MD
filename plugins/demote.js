    ...
    async execute(bot, msg, args) {
        ...
        try {
            ...
            if (!botMember.can_promote_members) { 
                return bot.sendMessage(chat.id, "I don't have permission to demote members.");
            }
            ...
            if (!['creator', 'administrator'].includes(admin.status)) {
             ...
             if (!admin.can_promote_members) {
                return bot.sendMessage(chat.id, "You don't have permission to demote members.");
            }
            ...
            if (!targetId) {
                return bot.sendMessage(chat.id, 'Please reply to the user, mention them, or provide their user ID to demote.');
            }
            ...
            await bot.sendMessage(chat.id, `Notifikasi: ${targetUsername} telah didemosikan dari admin.`);

        } catch (error) {
            await bot.sendMessage(chat.id, `❌ Failed to demote user. Error: ${error.message}`);
        }
    }
};
