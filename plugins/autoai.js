  ...
  async execute(bot, msg, args) {
    ...
    if (chat.type === 'private') {
        if (!isOwner(from.id)) {
            return bot.sendMessage(
                chat.id,
                'This command is restricted to the bot owner in private chat.'
            );
        }
    }
    ...
    if (chat.type !== 'private') {
        ...
        if (!userIsAdmin) {
            return bot.sendMessage(
                chat.id,
                'Only group admins can use this command in a group.'
            );
        }
    }

    const validProviders = ['groq', 'gemini', 'cerebras', 'agent'];
    let provider = 'groq';
    let state;
...
Default provider is groq if not specified.`,
    ...
    if (chatConfig.autoAi.provider !== provider && newEnabledState) {
        ...
        await bot.sendMessage(
            chat.id,
            `AI provider changed to *${provider}*. Chat history has been cleared.`,
            { parse_mode: 'Markdown' }
        );
    }
    ...
    await bot.sendMessage(
        chat.id,
        `AutoAI feature has been turned *${state.toUpperCase()}* for this chat with provider *${provider}*.`,
        { parse_mode: 'Markdown' }
    );
  }
};