...
const KEY = crypto.createHash('sha256').update(mainConfig.token).digest();
...
function decrypt(encryptedText) {
    try {
        ...
        print.error(error, 'Jadibot Decryption Failed');
        return null;
    }
}
    ...
    for (const file of files) {
        if (file.endsWith('.json')) {
            try {
                ...
                const token = decrypt(sessionData.token);
                const ownerId = decrypt(sessionData.ownerId);

                if (token && ownerId) {
                    setTimeout(() => {
                        startJadibot(token, ownerId, mainBotInstance);
                    }, delay);
                    delay += 1000;
                    count++;
                } else {
                    print.error(`Failed to decrypt session data for ${file}. Deleting corrupt session.`);
